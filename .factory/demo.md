...
Demo state is stored only in the separate browser database `demo:calendar-snapshotter`. It never reads or writes `calendar-snapshotter`, the real vault database. **Reset demo** deletes and reseeds only the demo database. **Start for real** leaves demo mode without copying its data.
