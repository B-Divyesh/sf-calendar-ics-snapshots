...
const BUNDLED_RELEASE: Release = {
  ...
  ].map((name) => ({ name, browser_download_url: `https://github.com/${REPO}/releases/download/v0.1.6/${name}` }))
};
...
async function resolveDownload(refresh = false): Promise<void> {
  ...
  try {
    ...
    applyRelease(BUNDLED_RELEASE);
    note.textContent = "The latest check was unavailable. The published v0.1.6 download is ready.";
  }
}
